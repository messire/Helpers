...
namespace Helpers.SideCore
{
	...
	public struct Side : IComparable, IComparable<bool>, IEquatable<bool>
	{
		...
		private const char MIN_VALUE = char.MinValue;
		...
		private static string TrimWhiteSpaceAndNull(string value)
		{
			...
			while (startIndex < value.Length && (char.IsWhiteSpace(value[startIndex]) || value[startIndex] == MIN_VALUE))
			{
				++startIndex;
			}

			while (index >= startIndex && (char.IsWhiteSpace(value[index]) || (int)value[index] == (int)MIN_VALUE))
			{
				--index;
			}
		...
		public static implicit operator Side(bool status) => new() { value = status };
